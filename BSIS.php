...
// Fetch BSIS program content
$bsisContent = ['title' => 'Bachelor of Science in Information System', 'content' => '', 'image_path' => ''];
$contentSql = "SELECT title, content, image_path FROM bsis_content LIMIT 1";
$contentResult = $conn->query($contentSql);
if ($contentResult && $contentResult->num_rows > 0) {
    $bsisContent = $contentResult->fetch_assoc();
}

// Fetch BSIS program events
$bsisEvents = [];
$eventsSql = "SELECT event_date, title, description, location FROM bsis_events ORDER BY event_date ASC";
...
<head>
    ...
    position: relative;
        ...
        .hero-section {
            position: relative;
            height: 400px;
            background: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.1))<?= !empty($bsisContent['image_path']) ? ", url('admin/" . htmlspecialchars($bsisContent['image_path']) . "') center/cover" : "" ?>;
            display: flex;
            align-items: center;
...
<body style="background-color: white;">
  ...
  <section class="hero-section">
    ...
    <section class="content-section">
        <h1 class="section-title"><?= htmlspecialchars($bsisContent['title']) ?></h1>
        <div class="section-description">
            <?= !empty($bsisContent['content']) ? $bsisContent['content'] : '' ?>
        </div>
    </section>
    ...
    <section class="events-section">
        <div class="events-container">
            <h2 class="section-title">BSIS Program Events</h2>
            <div class="events-grid">
                <?php if (!empty($bsisEvents)): ?>
                    <?php foreach ($bsisEvents as $event): ?>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <!-- No events in database - leave blank as requested -->
                    <p style="grid-column: 1/-1; text-align: center;">No upcoming events scheduled at this time.</p>
                <?php endif; ?>
            </div>
        </div>
    </section>
