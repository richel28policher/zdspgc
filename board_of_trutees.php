...
// Fetch board page content
$boardContent = ['title' => 'Board of Trustees', 'content' => '', 'image_path' => ''];
$contentSql = "SELECT title, content, image_path FROM board_page LIMIT 1";
$contentResult = $conn->query($contentSql);
if ($contentResult && $contentResult->num_rows > 0) {
    $boardContent = $contentResult->fetch_assoc();
}

// Fetch board members
$boardMembers = [];
$membersSql = "SELECT id, name, designation, details, image_path FROM board_members ORDER BY FIELD(designation, 'Chairperson', 'Vice Chairperson') DESC, name ASC";
    ...
    position: relative;
        ...
        .hero-section {
            ...
            background: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.1)), 
                        <?= !empty($boardContent['image_path']) ? "url('admin/" . htmlspecialchars($boardContent['image_path']) . "') center/cover" : "url('images/image1.jpg') center/cover" ?>;
            display: flex;
            align-items: center;
...
.dropdown:hover .dropdown-menu {
 ...
 .board-content {
        ...
        /* Special styling for chairperson */
        .chairperson .member-photo {
        ...
        /* Special styling for vice chairperson */
        .vice-chairperson .member-photo {
...
<body style="background-color: white;">
  ...
  <?php include('main-nav.php'); ?> 
     ...
     <section class="hero-section">
        <h1 class="hero-title">Board of Trustees</h1>
    </section>

    <div class="board-content">
        <div class="board-intro">
            <h2><?= htmlspecialchars($boardContent['title']) ?></h2>
            <div class="section-description"><?= $boardContent['content']; ?></div>
        </div>
            ...
            <div class="leadership-grid">
                <?php if (!empty($boardMembers)): ?>
                    <?php foreach ($boardMembers as $member): ?>
                        <?php 
                            ...
                            $isVice = stripos($member['designation'], 'Vice Chairperson') !== false;
                        ?>
                        <div class="board-member <?= $isChair ? 'chairperson' : ($isVice ? 'vice-chairperson' : '') ?>">
                            <div class="member-photo">
                                <?php if (!empty($member['image_path'])): ?>
                                    <img src="admin/<?= htmlspecialchars($member['image_path']) ?>" alt="<?= htmlspecialchars($member['name']) ?>">
                                <?php else: ?>
                            ...
                            <div class="member-info">
                                ...
                                <div class="member-title"><?= htmlspecialchars($member['designation']) ?></div>
                                <div class="member-details">
                                    <?= htmlspecialchars($member['details']) ?>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div style="grid-column: 1 / -1; text-align: center; padding: 40px;">
                        <p>No board members found. Please check back later.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
