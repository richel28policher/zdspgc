...
$calendars = [];
$sql = "SELECT id, school_year, file_path FROM master_calendar ORDER BY school_year DESC";
...
// Fetch hero content for calendar page
$heroContent = ['title' => '', 'content' => '', 'image_path' => ''];
$contentSql = "SELECT title, content, image_path FROM calendar_hero_content LIMIT 1";
$contentResult = $conn->query($contentSql);
if ($contentResult && $contentResult->num_rows > 0) {
    $heroContent = $contentResult->fetch_assoc();
}
        ...
        * {
        ...
        .hero-section {
            ...
            background: linear-gradient(rgba(0,0,0,0.1), rgba(0,0,0,0.1)), 
                        url('admin/<?= !empty($heroContent['image_path']) ? htmlspecialchars($heroContent['image_path']) : 'images/image1.jpg' ?>') center/cover;
            display: flex;
            align-items: center;
            justify-content: center;
            text-align: center;
...
<body style="background-color: white;">
    ...
    <section class="content-section">
        <h1 class="section-title"><?= htmlspecialchars($heroContent['title'] ?: 'Academic Calendar') ?></h1>
       
        
        <!-- School Calendar Buttons -->
        <div class="calendar-buttons">
            <?php foreach ($calendars as $calendar): ?>
                <a href="admin/<?= htmlspecialchars($calendar['file_path']) ?>" class="calendar-btn" target="_blank">
                    <?= htmlspecialchars($calendar['school_year']) ?> School Calendar
                </a>
            <?php endforeach; ?>
            <?php if (empty($calendars)): ?>
                <p>No calendar files available yet.</p>
            <?php endif; ?>
        </div>

        <!-- College Seal -->
        <div class="college-seal">
            <img src="images/zdspgc_logo.png" alt="ZDSPGC Seal" class="seal-image">
        </div>
    </section>
